    ...
    <div className="flex flex-col gap-4">
          ...
          {order.map((slug, i) => {
            ...
            return (
              <TR key={p.id}>
                ...
                <TD>
                  {p.label}
                  {p.is_custom && <span className="ml-1 text-xs text-ink-500">(custom)</span>}
                </TD>
                <TD>{p.kind}</TD>
                <TD>{p.model}</TD>
                <TD>
                ...
                <TD>
                  <Badge variant={p.enabled ? "brand" : "neutral"}>{p.enabled ? "enabled" : "disabled"}</Badge>
                </TD>
                <TD>
      ...
      >
        ...
        <Button type="submit" variant="secondary" size="sm" disabled={ordering}>
          {ordering ? "Saving order…" : "Save order"}
        </Button>
      ...
      <div className="rounded-lg border border-ink-200 bg-ink-50 p-3">
        <div className="mb-2 text-sm font-medium text-ink-700">Add a custom OpenAI-style provider</div>
        ...
        <p className="mt-2 text-xs text-ink-500">
          Keys are read from environment variables only — built-ins use GEMINI_API_KEY / GROQ_API_KEY /
          OPENROUTER_API_KEY; a custom provider uses FSMS_AI_KEY_&lt;SLUG&gt; (upper-cased). Nothing secret is
